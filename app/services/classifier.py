...
CATEGORY_TAXONOMY = """
- Cost of Goods Sold (COGS): Inventory, Manufacturing, Shipping
- Operating Expenses: Software & SaaS, Office Supplies, Utilities, Rent, Insurance
- Payroll & Benefits: Salaries, Health Insurance, 401k, Contractors
- Marketing & Advertising: Digital Ads, PR, Events
- Travel & Entertainment: Airfare, Hotel, Ground Transport, Meals, Client Entertainment
- Professional Services: Legal, Accounting, Consulting, Recruiting
- Financial Charges: Bank Fees, Interest Expense, Merchant Processing Fees
- Capital Expenditure: Equipment, Furniture, Software Licenses
- Revenue / Income: Product Sales, Service Revenue, Interest Income, Refunds
        ...
        for row in (resp.data or []):
            ...
            if hit:
                return dict(
                    ...
                    match_method="exact",
                    match_confidence=float(row.get("confidence_score", 1.0)),
                    matched_rule_id=row["id"],
                    matched_rule_tier=tier,
                )
    return None
...
async def _tier2_fuzzy(supabase, vendor, firm_id, client_id):
    try:
        ...
        if resp.data:
            best = resp.data[0]
            return dict(
                category_l1=best["category_l1"],
                category_l2=best.get("category_l2"),
                match_method="fuzzy",
                match_confidence=float(best["similarity"]),
                matched_rule_id=best["id"],
                matched_rule_tier=best["tier"],
            )
    except Exception as e:
        logger.warning("Fuzzy match failed: %s", e)
    return None

async def _tier3_llm(llm, transaction, vendor):
    prompt = f"""You are a senior CPA. Classify this bank transaction.
...
{{"category_l1": "<category>", "category_l2": "<subcategory or null>", "confidence": <0.0-1.0>, "reasoning": "<one sentence>"}}"""
    try:
        ...
        return dict(
            ...
            match_method="llm",
            match_confidence=float(data.get("confidence", 0.5)),
...
async def classify_one(supabase, llm, transaction, firm_id, client_id=None):
    ...
    if not result or result["match_confidence"] < 0.9:
        ...
        if fuzzy and fuzzy["match_confidence"] >= cfg.fuzzy_confidence_promote:
            ...
            result = await _tier3_llm(llm, transaction, vendor)
