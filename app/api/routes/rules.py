...
from app.middleware.auth import AuthContext, require_auth, require_admin
...
async def list_rules(
    ...
    # Client + Firm rules
    q = supabase.table("firm_vendor_categories").select("*").eq("firm_id", auth.firm_id).eq("is_active", True)
    firm_resp = await q.execute()
    ...
    if client_id:
        ...
        if client_resp.data and client_resp.data.get("industry"):
            ind_resp = await supabase.table("industry_vendor_categories").select("*") \
                .eq("industry", client_resp.data["industry"]).eq("is_active", True).execute()
...
async def update_rule(
    ...
    await supabase.table("firm_vendor_categories").update(updates) \
        .eq("id", rule_id).eq("firm_id", auth.firm_id).execute()
    return {"success": True}
...
async def delete_rule(
    ...
    await supabase.table("firm_vendor_categories").update({"is_active": False}) \
        .eq("id", rule_id).eq("firm_id", auth.firm_id).execute()